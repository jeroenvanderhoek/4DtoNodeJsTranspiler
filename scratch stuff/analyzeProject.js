// Count 4D-commands used in ./projectToAnalyze/
...
console.log("Count used 4d commands...");  

globSync("projectToAnalyze/**/*.4dm").forEach(function(entry){ 

    console.log(`Entry "${entry}`);

    let data = fs.readFileSync(entry, 'utf8');

    counts.forEach((cmdObject) => {
    
        // Create a regular expression to find all occurrences of the search string  
        const regex = new RegExp(cmdObject.cmd, 'g');  
        // Match the regex against the file content  
        const matches = data.match(regex);  
        
        // Count the occurrences  
        const count = matches ? matches.length : 0;  
    
        cmdObject.count += count;

        if ( count > 0) {
            console.log(`The string "${cmdObject.cmdName}" occurs ${count} times in `+ entry);
        }
...
counts.forEach((cmdObject) => {
    let paddingRight = "";
    for (let i = 0; i < columnWidth - cmdObject.cmdName.length; i++) {
        paddingRight += " ";
    }

    let paddingLeft = "";
    for (let i = 0; i < 10 - String(cmdObject.count).length; i++) {
        paddingLeft += " ";
    }

    table += `| ${ cmdObject.cmdName + paddingRight } | ${ cmdObject.count > 0 ? paddingLeft + cmdObject.count : "" } 
`;
});
